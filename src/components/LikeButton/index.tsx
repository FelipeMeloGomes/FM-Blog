...
const LikeButton = ({ postId, userId, onNotLoggedIn }: LikeButtonProps) => {
  ...
  return (
    <button
      className={`w-[140px] h-[40px] flex items-center justify-start border-none rounded-none overflow-hidden shadow-[5px_5px_10px_rgba(0,0,0,0.089)] cursor-pointer bg-transparent transition-colors duration-150 ease-in-out ${
        liked ? "bg-red-700" : ""
      } hover:focus:bg-red-800 active:bg-red-700`}
      onClick={handleLikeClick}
      disabled={!postId}
    >
      <span
        className={`w-[60%] h-full flex items-center justify-center gap-2 transition-colors duration-150 ease-in-out ${
